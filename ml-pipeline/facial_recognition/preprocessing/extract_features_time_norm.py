...
def aspect_corrected_landmarks(landmarks, width: int, height: int):
    """
    MediaPipe landmark.x/y는 각각 이미지 너비/높이로 독립 정규화된다. 이미지가
    정사각형이 아니면(R_live_clip/ATK_external_clip의 16:9 풀프레임 vs
    S_dataset_sequence의 정사각형 크롭) y축이 체계적으로 왜곡되어 _dist() 기반의
    모든 거리/각도(EAR, MAR, nose_movement, head_roll 등)가 source별 촬영 비율
    차이만으로 달라진다. 이것이 R_live_clip 오탐의 실제 원인이며, frame_interval
    시간 정규화(이 파일의 본래 목적)와는 별개의 버그다 (RETROSPECTIVE 참고).
    y를 너비 기준 단위로 환산해 이후 모든 계산이 단일 단위를 쓰도록 만든다.
    """
