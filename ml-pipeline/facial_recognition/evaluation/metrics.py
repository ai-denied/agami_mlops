"""
얼굴 활성도 모델 성능 지표 산출 유틸리티.

label convention: 0=live(genuine), 1=spoof(attack)
pred=1 → block/reject
FAR = FN / (TP+FN): 공격이 통과된 비율
FRR = FP / (TN+FP): 실사람이 거부된 비율
"""
...
def threshold_sweep(
    ...
    return pd.DataFrame([binary_metrics(y_true, prob_spoof, float(th)) for th in thresholds])
...
def choose_threshold(
    ...
    if strategy == "best_f1":
        return float(df.loc[df["f1_spoof"].idxmax(), "threshold"])

    if strategy == "eer_like":
        diff = (df["far_attack_pass_rate"] - df["frr_genuine_reject_rate"]).abs()
        return float(df.loc[diff.idxmin(), "threshold"])
...
def grouped_metrics(
    ...
    for g in sorted(pd.unique(groups)):
        ...
        m.update({"model": model_name, "split": split_name,
                  "group_name": group_name, "group_value": str(g)})
        rows.append(m)
