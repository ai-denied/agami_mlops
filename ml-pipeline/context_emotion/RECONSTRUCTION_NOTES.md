...
## 최종 라벨 목록 (2026-06-22 스키마 고정 — 아래 "스키마 고정" 절 참고)
...
**상황 8종**: conflict, danger, everyday, loss_absence, pressure, safety,
teasing, vanity

이 14+8은 `context_emotion_train_dataset_v1` 빌드를 시작하면서 사용자가 명시적으로
고정한 스키마다. 그 이전 단계에서는 감정 15종(empathy 포함)·상황 7종(everyday 제외)
버전을 거쳤었는데, 그 변경 경위는 바로 아래 "2026-06-22 스키마 고정" 절에 있다.
