...
3. v1 학습셋을 만들면서 사용자가 **최종적으로 감정 14종(empathy 제외) +
   상황 8종(everyday 포함)**으로 다시 고정했다. 이번이 최종 스키마다.
...
**최종 상황 8종**: conflict, danger, everyday, loss_absence, pressure,
safety, teasing, vanity (everyday는 스키마에 있지만 실제 매핑 데이터는
0건)
...
(`doubt_confusion`, `doubt_confusning` 등)를 흡수해 14종/8종 캐노니컬
...
- emotion/situation 값이 14종/8종에 없음 → `*_label_not_in_schema`
...
| 상황 8종 합계 | 128건 (전부 manual; everyday는 0건) |
...
5. **everyday 클래스는 데이터가 0건이다.** 스키마에는 있지만 실제로
   채울 데이터 소스가 없어 향후 보강이 필요하다.
